async function get_nearest(lat: number, long: number): Promise<stationdata[]> {
    const station_data = await fetch(`https://tankstellenfinder.aral.de/api/v1/locations/nearest_to?lat=${lat}&lng=${long}&autoload=true&travel_mode=driving&avoid_tolls=false&avoid_highways=false&show_stations_on_route=true&corridor_radius=5&format=json`);
...
const filtered_station_data: stationdata[] = original_station_data.filter((station) => (station.country_code == "DE"));
